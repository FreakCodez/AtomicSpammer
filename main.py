...
class Spammer:
    ...
    def main_menu(self):
        clear()
        self.logo()
        print(f'[{Fore.LIGHTCYAN_EX}1{reset}] Webhook Spammer        [{Fore.LIGHTCYAN_EX}2{reset}] Bot Token Nuker        [{Fore.LIGHTCYAN_EX}3{reset}] Bot Token Spammer')
        choice = int(input(f'\n{self.pref}'))
        if choice == 1:
            clear()
            self.webhookspammer()
        ...
        while True:
            try:
                ...
                if 200 or '200' in response:
                    tcount += 1
                    print(f'{self.success}Sent "@everyone {msg}" to webhook! | Total Count: {tcount}')
    ...
    def tkennuker(self):
        ...
        try:
                ...
                input('Could not login to bot, Please check the token. Press enter to go back.')
                self.main_menu()
